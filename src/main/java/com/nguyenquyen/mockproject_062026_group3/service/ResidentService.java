...
public class ResidentService {
  ...
  public Page<ResidentResponse> getResidents(String status, Long bedId, String search, int page,
      int pageSize) {
    Pageable pageable = PageRequest.of(page - 1, pageSize);
    Page<Resident> residentPage = residentRepository.findResidentsFiltered(status, bedId, search,
        pageable);
  ...
  public ResidentResponse updateResident(Long id, ResidentUpdateRequest request, String userRole) {
    ...
    if (resident.getIsChartLocked() != null && resident.getIsChartLocked() && "CNA".equals(
        userRole)) {
    ...
    if (request.getMaritalStatus() != null) {
      resident.setMaritalStatus(request.getMaritalStatus());
    }
    if (request.getReligionPreference() != null) {
      resident.setReligionPreference(request.getReligionPreference());
    }
  ...
  public LocStatusResponse getLocStatus(Long residentId) {

    ResidentCareLevelHistory history =
        historyRepository
            .findFirstByResidentIdAndEndDateIsNullOrderByStartDateDesc(residentId)
            .orElse(null);

    if (history == null) {

      return LocStatusResponse.builder()
          .confirmed(false)
          .build();
